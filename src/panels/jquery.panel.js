...
$( function()
{
	...
	$.widget( 'jqueryDash.panel', $.jqueryDash.localStorageEngine, {
		...
		_create: function()
		{
			...
			this.options.width = ( this.isSetting( 'width' ) ) ? this.setting( 'width' ) : this.options.minWidth;
			this.options.height = ( this.isSetting( 'height' ) ) ? this.setting( 'height' ) : this.options.minHeight;
		...
		_drawEditPanel: function()
		{
			...
			if( this.options._editDialogCompatible )
			{
				this._toggleOpenEditorDialog();
			}
			...
			if( this.options._resizable )
			{
				...
				this.element.resizable( {
					...
					stop: function()
					{
						...
						self.setting( 'width', self.options.width );
						self.setting( 'height', self.options.height );
						if( self.options._editDialogCompatible )
						{
							self._toggleOpenEditorDialog();
						}
					}
				} );
			}
			...
			{
				this.element.append(
					$( '<a/>', {'text': 'edit', 'class': 'open-edit-dialog'} ).click( function()
					{
						self.element.find( '.content' ).clone( true, true ).dialog(
							{
								width: self.options._minEditorSize.w + 50,
								height: self.options._minEditorSize.h + 100,
		...
		_destroyEditPanel: function()
		{
			...
			// Detach the dom object for speedy manipulation
			this.element.detach();

			// Destroy optional edit mode widgets
			if( this.options._resizable )
			{
				this.element.resizable( 'destroy' );
			}

			// Finish cleaning up edit mode a re-attach the dom object
		...
		addEditorField: function( fieldType, settingName, defaultValue, fieldLabel, additionalData )
		{
			...
			if( this.options._hiddenEditorFields[ settingName ] !== undefined )
			{
				return;
			}
		...
		{
			this.options._hiddenEditorFields[ settingName ] = true;
		...
		_drawEditorFields: function( clearContent )
		{
			...
			for( fieldSetting in this.options._editorFields )
			{
				if( this.options._editorFields.hasOwnProperty( fieldSetting ) )
				{
					...
					container.append(
						$( '<label/>', { 'html': fieldSettings.label } ).append( input.addClass( 'ef ' +
																								 fieldSettings.editorId ) )
					);
				}
			}
		},
		...
		/**
		 * Adds or removes the class used to determine if edit in dialog link should be shown
		 *
		 * @private
		 */
		_toggleOpenEditorDialog: function()
		{
			this.element.toggleClass(
				'dialog-edit',
				this.options.width < this.options._minEditorSize.w ||
				this.options.height < this.options._minEditorSize.h
			);
		}
	...
	$( 'body' ).on( 'change.jqueryDash', 'select.ef', function( event )
	{
		// Set the selected attribute so that if the element is moved the value will be maintained
		$( this )
			// remove old select element attribute
			.children( '[selected]' ).removeAttr( 'selected' ).end()
			// set new attribute for elements with selected property
			.children( ':selected' ).attr( 'selected', true );
	} );
