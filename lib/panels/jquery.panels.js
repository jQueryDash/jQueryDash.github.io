	...
	$.widget( 'jqueryDash.panels', $.jqueryDash.localStorageObject, {
		...
		_create: function()
		{
			...
			this.element.append(
				...
				$( '<a/>', { 'class': 'button settings above-panels', 'html': '&#9883;', 'title': 'Settings' } )
					.click( function()
					{
						...
						$( '<div/>', {'class': 'settings'} )
							...
							.append(
								$( '<div/>', {'class': 'import-export pane'} )
									.append(
										$( '<label/>', {'text': 'Export Settings'} ).append(
		...
		_addPanel: function( id, panelType, newPanel )
		{
			...
			this.element.append( this._panelElements[ id ][ panelType ](
				( newPanel !== undefined && newPanel ) ? { 'controller': this, 'id': id, 'mode': 'edit', 'ordering': id } : { 'controller': this, 'id': id }
			) );
		},
		...
		/**
		 * Exports the state of the application to a json file
		 *
		 * @private
		 */
		_exportSettings: function()
		{
			// Create a pretend link and click it to do a download
			var fakeLink = document.createElement( 'a' );
			fakeLink.setAttribute( 'href', 'data:application/json;charset=utf-8,' +
										   encodeURIComponent( JSON.stringify( localStorage ) ) );
			fakeLink.setAttribute( 'download', 'settings.json' );
			fakeLink.click();
		},
