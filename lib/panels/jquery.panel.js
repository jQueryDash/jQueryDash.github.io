...
$( function()
{
	...
	$.widget( 'jqueryDash.panel', $.jqueryDash.localStorageObject, {
		...
		_create: function()
		{
			...
			initialWidth = ( this.isSetting( 'width' ) ) ? this.setting( 'width' ) : this.options.position.w;
			initialHeight = ( this.isSetting( 'height' ) ) ? this.setting( 'height' ) : this.options.position.h;
		...
		{
			this.element.append( $( '<div>', { 'class': 'content', 'text': 'content' } ) );
		},
		 ...
		 * @private
		 */
		_addEditorField: function( fieldType, settingName, defaultValue, fieldLabel )
		{
			...
			this._editorFields[ settingName ] = {
				'type': fieldType,
				'label': fieldLabel,
				'editorId': 'ef' + this._editorFieldCount++
			};
		...
		_drawEditorFields: function( clearContent )
		{
			...
			for( fieldSetting in this._editorFields )
			{
				if( this._editorFields.hasOwnProperty( fieldSetting ) )
				{
					fieldSettings = this._editorFields[ fieldSetting ];
					...
					switch( fieldSettings.type )
					{
						case 'bool':
							...
							$( '<input>',
								{
									'type': 'checkbox',
									'checked': ( parseInt( this.setting( fieldSetting ), 10 ) === 1 )
								}
							);
							break;
						default:
		...
		_saveEditorFieldValues: function()
		{
			...
			for( fieldSetting in this._editorFields )
			{
				if( this._editorFields.hasOwnProperty( fieldSetting ) )
				{
					...
					{
						case 'bool':
							settingValue = container.find( '.' + fieldSettings.editorId ).is( ':checked' ) ? 1 : 0;
							break;
						default:
