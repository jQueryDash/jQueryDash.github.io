...
$( function()
{
	...
	$.widget( 'jqueryDash.panel', $.jqueryDash.localStorageObject, {
		...
		_destroyViewPanel: function()
		{
			...
			if( this.options.currentRequest !== null )
			{
				this.options.currentRequest.abort();
			}
		},
		...
		sendGetRequest: function( url, contentType, onReadyStateChange, enableCORSProxy )
		{
			...
			this.options.currentRequest = new XMLHttpRequest();
			...
			this.options.currentRequest.onreadystatechange = function()
			{
			...
			this.options.currentRequest.open(
				'get',
				enableCORSProxy ? 'http://www.corsproxy.com/' + url.replace( /^http:\/\//i, '' ) : url,
				true
			);
			this.options.currentRequest.setRequestHeader( 'Accept', contentType );
			if( enableCORSProxy )
			{
				this.options.currentRequest.setRequestHeader( 'Content-type', contentType + '; charset=UTF8' );
			}
			this.options.currentRequest.send();
		}
