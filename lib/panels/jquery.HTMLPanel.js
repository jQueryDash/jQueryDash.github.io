...
$( function()
{
	...
	var editControlsDivName = 'editControls';
	...
	$.widget( 'jqueryDash.HTMLPanel', $.jqueryDash.panel, {
		...
		_drawEditPanel: function()
		{
			...
			if( $( '#' + editControlsDivName ).length === 0 )
			{
				this.options.controller.element.append( '<div id="' + editControlsDivName + '"><div>\
				...
				$( '#' + editControlsDivName + ' a' ).click( function( event )
				{
	...
	// HTMLPanel Static Methods
	$.jqueryDash.HTMLPanel.showEditControls = function()
	{
		$( '#' + editControlsDivName ).show();
	}

	$.jqueryDash.HTMLPanel.hideEditControls = function()
	{
		$( '#' + editControlsDivName ).hide();
	}
	...
	{
		if( $( event.toElement ).parents( '.HTMLPanel' ).length > 0 || $( event.toElement ).parents( '#editControls' ).length > 0 )
		{
			$.jqueryDash.HTMLPanel.showEditControls();
		}
		else
		{
			$.jqueryDash.HTMLPanel.hideEditControls();
		}
	});
} );