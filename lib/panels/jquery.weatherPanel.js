$( function()
{
	...
	$.widget( 'jqueryDash.weatherPanel', $.jqueryDash.panel,
	{
		...
		refreshContent: function()
		{
			...
			{
				var responseJson, icon, description, temp, wind, tempIcon;
				...
				tempIcon = $( '<div />', { id: "weather-temp-icon" } );
				temp = $( '<div />', { id: "weather-temp" } ).text( parseInt( responseJson.main.temp, 10 ) );
				temp.append( $( "<span />", { 'class': "weather temp-unit" } ).text( "C" ) );
				icon = $( '<div />', { id: "weather-icon", 'class': "weather " + self.getWeatherIconClass( responseJson.weather[0].icon ) } );
				tempIcon.append( temp ).append( icon );
				...
				self.element.find( 'div.content' ).text( '' ).append( tempIcon ).append( description ).append( wind );
			};
